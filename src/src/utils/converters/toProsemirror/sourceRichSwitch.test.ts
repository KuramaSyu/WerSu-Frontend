...
// Pins cycle stability (cycle N+1 == cycle N) and structural preservation
// of headings, tables, and fenced code blocks across parse/render cycles.
...
// Joined at runtime so we don't have to escape the backticks inside
// the raw markdown string (which would make it unreadable).
const FENCE = "```";
...
// Uses the shared `renderTableToMarkdown` helper from `TableControlls`.
// The Table.extend stub below exists because `TableControlls.tsx` itself
// drags in MUI via `ReactNodeViewRenderer`, so we cannot import the
// production extension here. The renderMarkdown function it points at is
// the same one the production code uses.
